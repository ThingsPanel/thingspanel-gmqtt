...
type Device struct {
	...
	DeviceConfigID  *string    `gorm:"column:device_config_id;comment:设备配置id（外键）" json:"device_config_id"`          // 设备配置id（外键）
	BatchNumber     *string    `gorm:"column:batch_number;comment:批次编号" json:"batch_number"`                        // 批次编号
	ActivateAt      *time.Time `gorm:"column:activate_at;comment:激活日期" json:"activate_at"`                          // 激活日期
	IsOnline        int16      `gorm:"column:is_online;not null;default:0;comment:是否在线 1-在线 0-离线" json:"is_online"` // 是否在线 1-在线 0-离线
	AccessWay       *string    `gorm:"column:access_way;comment:接入方式A-通过协议 B通过服务" json:"access_way"`                // 接入方式A-通过协议 B通过服务
	Description     *string    `gorm:"column:description;comment:描述" json:"description"`                            // 描述
	ServiceAccessID *string    `gorm:"column:service_access_id;comment:服务接入id" json:"service_access_id"`            // 服务接入id
