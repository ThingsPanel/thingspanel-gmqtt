...
- 示例：设备发布 `sj23hih12l4/up/2536548`，配置原始主题为 `+/up/+`，映射目标主题为 `devices/telemetry`。

### 下行原始主题
- 定义：设备实际订阅主题，必须包含 `{device_number}`；当目标主题末尾含 `+` 时，原始主题可对应带 `+`（表示消息 ID 或请求 ID）。
- 示例：设备订阅 `sj23hih12l4/down`，配置原始主题 `{device_number}/down`，映射到目标主题 `devices/telemetry/control/{device_number}`。
