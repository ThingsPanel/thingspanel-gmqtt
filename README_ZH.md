...
# 项目现状
这个项目目前由 **ThingsPanel 团队维护**，我们会**持续修复 Bug、响应并合并社区提交的 issues 和 PR**，确保项目保持稳定和更新。

**使用情况：**
该项目已在 **超过十万台设备** 的生产环境中稳定运行。
在这些环境中，每个客户端每 **5 秒** 发布一次 **QoS 1** 消息以上报状态，云端订阅者会将这些状态数据持久化存储。
所有客户端均为非持久会话（non-persistent session），并使用 **MQTT v3.1.1 协议**。

我们仍建议在将本项目部署到生产环境前，**根据自身需求充分评估与测试**。
